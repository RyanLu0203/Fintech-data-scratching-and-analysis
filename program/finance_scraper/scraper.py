...
SINA_BULLETIN_URL = "https://vip.stock.finance.sina.com.cn/corp/go.php/vCB_AllBulletin/stockid/{code}.phtml"
SINA_BASE_URL = "https://vip.stock.finance.sina.com.cn"
...
def plain_symbol_name_variants(symbol: str) -> set:
...
def looks_like_symbol_name(value: str, symbol: str) -> bool:
    text = clean_text(str(value or "")).upper()
    if not text:
        return True
    code = plain_stock_code(symbol)
    stripped = code.lstrip("0") or code
    if text in plain_symbol_name_variants(symbol):
        return True
    return bool(text.isdigit() and (text == stripped or text.zfill(6) == code))


def load_stock_aliases() -> Dict[str, str]:
    path = os.path.join(PROJECT_ROOT, "config", "stock_aliases.json")
    ...
    try:
        ...
        return {}
    return {plain_stock_code(str(key)): clean_text(str(value)) for key, value in payload.items() if clean_text(str(value))}


def alias_company_name(symbol: str) -> str:
    return load_stock_aliases().get(plain_stock_code(symbol), "")


def resolve_company_name_for_symbol(symbol: str, company_name: Optional[str]) -> str:
    alias = alias_company_name(symbol)
    candidate = clean_text(str(company_name or ""))
    if alias and candidate and candidate != alias and candidate in set(load_stock_aliases().values()):
        print(
            f"Company name '{candidate}' does not match {plain_stock_code(symbol)}; using configured alias '{alias}'.",
            file=sys.stderr,
        )
        return alias
    return candidate or alias
...
def request_bytes(
    ...
    url = build_url(url, params)
    request_headers = headers or browser_headers(url, accept="html")
    req = urllib.request.Request(url, headers=request_headers)
    last_error: Optional[Exception] = None
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                retry_pause(attempt)
    try:
        return request_bytes_with_curl(url, headers=request_headers)
    except Exception:
        raise RuntimeError(f"Byte request failed after {retries} attempts: {url}\n{last_error}")


def request_bytes_with_curl(url: str, headers: Optional[Dict[str, str]] = None) -> bytes:
    curl = shutil.which("curl")
    if not curl:
        raise RuntimeError("curl is not available")
    cmd = [
    ...
    if completed.returncode != 0:
        raise RuntimeError(completed.stderr.decode("utf-8", errors="replace").strip() or f"curl exited {completed.returncode}")
    return completed.stdout
...
def request_json_post(
    ...
    request_headers = headers or browser_headers(url)
    request_headers = {**request_headers, "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                retry_pause(attempt)
    try:
        body = request_text_with_curl(
            url,
            headers=request_headers,
            # This fallback is intentionally GET-only; most CNINFO failures are
            # transient, so surface the POST error for diagnostics instead.
        )
        return json.loads(body)
    except Exception:
        raise RuntimeError(f"POST JSON request failed after {retries} attempts: {url}\n{last_error}")
        ...
        if isinstance(value, list) and value:
            for item in value:
                if isinstance(item, str) and re.search(r"[\u4e00-\u9fff]", item):
                    ...
                    if not looks_like_symbol_name(candidate, symbol):
                        return candidate
...
def fetch_eastmoney_announcements_between(
    ...
    page_size = 100
    max_pages = adaptive_news_max_pages(start_date, end_date, count, page_size, hard_cap=120)
    deep_history_scan = should_deep_history_scan(start_date, end_date)
    ...
    for page in range(1, max_pages + 1):
        ...
        for item in items:
            ...
            if len(normalized) >= count and not deep_history_scan:
                return date_balanced_event_sample(deduplicate_events(normalized), start_date, end_date, count)
...
def fetch_cninfo_announcements_between(
    symbol: str,
    start_date: dt.date,
    end_date: Optional[dt.date],
    count: int,
    ...
    if count <= 0:
        return []
    code = plain_stock_code(symbol)
    top = request_json_post(
        CNINFO_TOP_SEARCH_URL,
        {"keyWord": code, "maxSecNum": 10},
        retries=retries,
        headers=browser_headers(CNINFO_TOP_SEARCH_URL, referer="http://www.cninfo.com.cn/"),
    )
    matches = top if isinstance(top, list) else []
    match = next((item for item in matches if str(item.get("code", "")) == code), None)
    org_id = str((match or {}).get("orgId", "")).strip()
    stock_param = f"{code},{org_id}" if org_id else code
    effective_end = end_date or dt.datetime.now(tz=dt.timezone.utc).date()
    page_size = 50
    max_pages = adaptive_news_max_pages(start_date, effective_end, count, page_size, hard_cap=40)
    normalized: List[Dict[str, Any]] = []
    ...
    for page in range(1, max_pages + 1):
        payload = request_json_post(
            CNINFO_ANN_URL,
            {
                ...
                "category": "",
                "trade": "",
                "seDate": f"{start_date.isoformat()}~{effective_end.isoformat()}",
                "sortName": "",
                "sortType": "",
                "isHLtitle": "true",
            },
            retries=retries,
            headers=browser_headers(CNINFO_ANN_URL, referer="http://www.cninfo.com.cn/"),
        )
        ...
        for item in items:
            ...
            if millis in {"", None}:
                continue
            date = dt.datetime.fromtimestamp(float(millis) / 1000).date().isoformat()
            if date < start_date.isoformat():
                old_seen += 1
                continue
            if date > effective_end.isoformat():
                continue
            ...
            normalized.append(
                {
                    "date": date,
                    "title": title,
                    "summary": f"巨潮资讯公告；证券简称：{clean_text(item.get('secName') or '')}",
                    "publisher": "巨潮资讯",
                    "link": urllib.parse.urljoin(CNINFO_STATIC_URL, adjunct),
                    "published_utc": date,
                    "source_type": "external",
                }
            )
        if len(normalized) >= count or old_seen == len(items):
            break

    return date_balanced_event_sample(deduplicate_events(normalized), start_date, effective_end, count)


def fetch_sina_bulletins_between(
    symbol: str,
    start_date: dt.date,
    end_date: Optional[dt.date],
    count: int,
    ...
    normalized: List[Dict[str, Any]] = []
    page_size_guess = 30
    max_pages = adaptive_news_max_pages(start_date, effective_end, count, page_size_guess, hard_cap=20)

    for page in range(1, max_pages + 1):
        url = SINA_BULLETIN_URL.format(code=code)
        if page > 1:
            url = f"{url}?Page={page}"
        raw = request_bytes(url, retries=retries, headers=browser_headers(url, accept="html", referer="https://finance.sina.com.cn/"))
        ...
        if not matches:
            break
        old_seen = 0
        for date, href, title_html in matches:
            if date < start_date.isoformat():
                old_seen += 1
                continue
            if date > effective_end.isoformat():
            ...
            link = urllib.parse.urljoin(SINA_BASE_URL, href)
            normalized.append(
                {
                    "date": date,
                    "title": title,
...
def fetch_eastmoney_stock_news_between(
    ...
    return date_balanced_event_sample(deduplicate_events(combined), start_date, end_date, count)
...
def fetch_eastmoney_stock_news_for_keyword(
    ...
    if count <= 0:
        return []
    page_size = 100
    max_pages = adaptive_news_max_pages(start_date, end_date, count, page_size, hard_cap=160)
    deep_history_scan = should_deep_history_scan(start_date, end_date)
    normalized: List[Dict[str, Any]] = []
...
def should_deep_history_scan(start_date: dt.date, end_date: Optional[dt.date]) -> bool:
    """Return True when the requested window is long enough to need older pages."""

    effective_end = end_date or dt.datetime.now(tz=dt.timezone.utc).date()
    return (effective_end - start_date).days > 365


def adaptive_news_max_pages(
    start_date: dt.date,
    end_date: Optional[dt.date],
    count: int,
    page_size: int,
    ...
    """Increase Eastmoney search depth for multi-year ranges without changing CLI usage."""

    effective_end = end_date or dt.datetime.now(tz=dt.timezone.utc).date()
    span_days = max(0, (effective_end - start_date).days)
    cap_pages = max(1, (max(count, 1) + page_size - 1) // page_size + 2)
    history_pages = max(1, int(span_days / 14) + 8)
    return max(1, min(hard_cap, max(cap_pages, history_pages)))


def date_balanced_event_sample(
    events: List[Dict[str, Any]],
    start_date: dt.date,
    end_date: Optional[dt.date],
    count: int,
) -> List[Dict[str, Any]]:
    """Keep external events spread across dates instead of letting recent spikes dominate."""

    if count <= 0:
        return []
    if len(events) <= count:
        return sorted(events, key=lambda item: (item.get("date", ""), item.get("title", ""), item.get("link", "")))
    ...
    for item in events:
        ...
        if not date:
            continue
        if date < start_date.isoformat() or date > effective_end.isoformat():
            continue
    ...
    unique_dates = max(1, len(by_date))
    soft_per_day_cap = max(1, min(25, int(count / unique_dates) + 2))
    selected: List[Dict[str, Any]] = []

    for rank in range(soft_per_day_cap):
        for date in sorted(by_date):
            items = sorted(by_date[date], key=lambda item: (item.get("title", ""), item.get("link", "")))
            if rank < len(items):
                selected.append(items[rank])
                if len(selected) >= count:
                    return deduplicate_events(selected)[:count]

    for date in sorted(by_date):
        for item in by_date[date]:
            selected.append(item)
            unique = deduplicate_events(selected)
            if len(unique) >= count:
                return unique[:count]
...
def main() -> int:
    ...
    args.company_name = resolve_company_name_for_symbol(symbol, args.company_name)
    ...
    if source_used in {"eastmoney", "tencent"}:
        resolved_company_name = args.company_name or chart_meta.get("longName", "") or alias_company_name(symbol)
        if looks_like_symbol_name(resolved_company_name, symbol):
            resolved_company_name = alias_company_name(symbol)
        quote_meta = {
            "symbol": symbol,
            "longName": resolved_company_name,
    ...
    if source_used in {"eastmoney", "tencent"}:
        ...
        try:
            ...
            try:
                cninfo_news = fetch_cninfo_announcements_between(symbol, start_date, end_date, args.news_count, args.retries)
                supplemental_news.extend(cninfo_news)
                print(f"CNINFO supplemental announcements: {len(cninfo_news)}")
            except Exception as exc:
                print(f"Warning: CNINFO supplemental fetch failed: {exc}", file=sys.stderr)
            try:
                sina_bulletins = fetch_sina_bulletins_between(symbol, start_date, end_date, args.news_count, args.retries)
                supplemental_news.extend(sina_bulletins)
                print(f"Sina supplemental bulletins: {len(sina_bulletins)}")
            except Exception as exc:
                print(f"Warning: Sina supplemental fetch failed: {exc}", file=sys.stderr)
            news = date_balanced_event_sample(
                deduplicate_events(stock_news + announcements + supplemental_news),
                start_date,
                end_date,
                args.news_count,
            )
            ...
            if missing_dates:
                ...
                news = date_balanced_event_sample(
                    deduplicate_events(news + market_news),
                    start_date,
                    end_date,
                    args.news_count,
                )
            ...
            if not quote_meta.get("longName") or looks_like_symbol_name(str(quote_meta.get("longName")), symbol):
                quote_meta["longName"] = alias_company_name(symbol) or infer_company_name_from_events(news, "")
        except Exception as exc:
    ...
    external_items = sum(1 for item in news if item.get("source_type", "external") == "external")
    generated_items = sum(1 for item in news if item.get("source_type") == "generated_ohlcv_summary")
    external_dates = len({item["date"] for item in news if item.get("source_type", "external") == "external"})
    generated_dates = len({item["date"] for item in news if item.get("source_type") == "generated_ohlcv_summary"})
    ...
    print(
        ...
        f"external={external_items} items/{external_dates} dates; "
        f"generated_ohlcv_summary={generated_items} items/{generated_dates} dates."
    )
    return 0
